const initialize = () => {
	cleanTable()
	fetch('/api/employees').then((res) => res.json()).then((res) => fillTable(res.employees))
}
...
const sendInfo = () => {
	// La validación es super descriptiva para el usuario y eso es excelente, pero creo que se podria simplificar un poco la estructura.
...
const searchEmployee = () => {
	...
	fetch(`/api/employees/${input}`).then((res) => res.json()).then((res) => console.log(res))
}

const filterById = () => {
	innerHTMLCleaner('idNotFound')
	const filter = document.getElementById('filter').value
	fetch(`/api/employees/${filter}`).then((res) => res.json()).then((res) => {
...
const openEdit = (index) => {
	fetch(`/api/employees/${index}`).then((res) => res.json()).then((res) => {
...
const editEmployee = () => {
	// se repite la edición igual a como esta en la creación y también los elementos en pantalla. podriamos intentar componetizarlo un poco.
